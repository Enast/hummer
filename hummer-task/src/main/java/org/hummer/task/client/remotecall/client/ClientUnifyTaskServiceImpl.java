...
public class ClientUnifyTaskServiceImpl implements ClientUnifyTaskService {
    ...
    public TaskAjaxResult<Boolean> reportTaskStatus(UnifyTaskStatus taskStatus) {
        TaskAjaxResult<Boolean> result = rt.exchange("http://cmdb/cmdb/cloud/unifyTask/v1/taskStatus", HttpMethod.POST, HttpUtils.getHttpEntity(taskStatus), new ParameterizedTypeReference<TaskAjaxResult<Boolean>>() {}).getBody();
        return result;
    }
    ...
    public TaskAjaxResult<Boolean> tasksRegister(List<UnifyTaskRegister> registers) {
        TaskAjaxResult<Boolean> result = rt.exchange("http://cmdb/cmdb/cloud/unifyTask/v1/tasksRegister", HttpMethod.POST, HttpUtils.getHttpEntity(registers), new ParameterizedTypeReference<TaskAjaxResult<Boolean>>() {}).getBody();
        return result;
    }
    ...
    public TaskAjaxResult<Boolean> tasksUpdate(List<UnifyTaskUpdate> tasks) {
        setServer(tasks);
        TaskAjaxResult<Boolean> result = rt.exchange("http://cmdb/cmdb/cloud/unifyTask/v1/tasksUpdate", HttpMethod.POST, HttpUtils.getHttpEntity(tasks), new ParameterizedTypeReference<TaskAjaxResult<Boolean>>() {}).getBody();
        return result;
    }
    ...
    public TaskAjaxResult<BasicTask> taskByNo(String taskNo) {
        ...
        TaskAjaxResult<BasicTask> result = rt.exchange("http://cmdb/cmdb/cloud/unifyTask/v1/findByNo?taskNo=" + taskNo + "&server=" + server, HttpMethod.GET, HttpUtils.getHttpEntity(null), new ParameterizedTypeReference<TaskAjaxResult<BasicTask>>() {}).getBody();
        return result;
    }

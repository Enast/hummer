...
import static org.enast.hummer.stream.flink.constant.PropertiesConstants.DEFAULT_KAFKA_BROKERS;
import static org.enast.hummer.stream.flink.constant.PropertiesConstants.DEFAULT_KAFKA_GROUP_ID;
import static org.enast.hummer.stream.flink.constant.PropertiesConstants.DEFAULT_KAFKA_ZOOKEEPER_CONNECT;
...
public class KafkaConfigUtil {
    ...
    public static DataStreamSource<MetricEvent> buildSource(StreamExecutionEnvironment env, String topic, Long time) throws IllegalAccessException {
        ...
        FlinkKafkaConsumer011<MetricEvent> consumer = new FlinkKafkaConsumer011<>(
                topic,
                new CommonSchema<MetricEvent>(MetricEvent.class),
                props);
        //重置offset到time时刻
        if (time != 0L) {
